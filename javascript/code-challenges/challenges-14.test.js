...
-- ['Mr. Brown', ' Ms. Red', 'Dr. Blue', 'Mrs.', '', 'Ms. Black', 'dr. Green', 'Mrs. Orange', 'Purple', 'Mr.  Pink'];
-----------------------------------------------------------------------------------
...
const screenForNames = (arr) => {
  ...
  //   let regexW = /^(Mr. ||Mrs. ||Ms. ||Dr. )/g;
  //   let resW = regexW.test(ele);
  //   return resW;
  // });

  let newArry = arr.filter((ele) => {
    let regexW = /^(Mr.|Mrs.|Ms.|Dr.) /g;
    let resW = regexW.test(ele);
    console.log(ele, resW);

    if (resW == 1) {
      return ele;
    }
  });
  let newArry2 = newArry.filter((eleA) => {
    let newReg = /^(Mr.|Mrs.|Ms.|Dr.)\s+ /g;
    let resW = newReg.test(eleA);
    if (resW == 0) {
      return eleA;
    }
  });
...
const toTitleCase = (arr) => {
  // Solution code here...
  let newArry = arr.map((ele) => {
    return ele.replace(/(^\w|\s\w)/g, (m) => m.toUpperCase());
  });
  return newArry;
};
...
let biggerThanLuke = (arr) => {
  ...
  let text = names.toString().replace(",", " - ");
  return text;
};
